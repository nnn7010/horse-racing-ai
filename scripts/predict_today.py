...
def estimate_ability(number: int, win_prob: float, place_prob: float,
    ...
    burst     = spread(base)
    course    = spread(base)
    form      = min(100, spread(int(place_prob * 120)))
    stability = spread(base - 5)
    jockey    = spread(base)

    return {
        "speed": speed, "burst": burst, "power": power,
        "course": course, "form": form, "stability": stability,
        "jockey": jockey, "has_data": True, "fit": spread(base),
    }


def estimate_running_style(number: int, win_prob: float, bracket: int) -> str:
    """簡易脚質推定（枠番ベースのヒューリスティック）"""
    import random
    ...
    for i in range(m):
        ...
        for j in range(m):
            ...
            for k in range(m):
                ...
                p3 = pk / s2
                prob_ijk = p1 * p2 * p3
                key3 = (int(nums_arr[i]), int(nums_arr[j]), int(nums_arr[k]))
